...
export function buildSystemPrompt(A: string, B: string, language: string): string {
  ...
  return [
    ...
    `"s6":{"a_card":"what closeness means to ${A}","b_card":"what closeness means to ${B}","tag":"verdict","blocks":[{"label":"Getting close","text":"how intimacy builds between these two specifically"},{"label":"What blocks it","text":"the honest obstacle and how it shows up"}]},`,
