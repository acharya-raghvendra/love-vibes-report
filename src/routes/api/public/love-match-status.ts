    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        const { data: order } = await supabaseAdmin
          ...
          .select("status, pdf_url, error_message, attempt_count, whatsapp_sent, email_sent, language, final_price")
          .eq("order_id", orderId)
          .maybeSingle();
        ...
        return Response.json(
          {
            ...
            // Non-PII order value (rupees) so the success page can report an
            // accurate conversion value. Price logic is unchanged.
            amount: typeof order.final_price === "number" ? order.final_price : null,
